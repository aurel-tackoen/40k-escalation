...
must_haves:
  ...
  artifacts:
    ...
    - path: "app/components/LeagueSwitcher.vue"
      ...
      contains: "Phase {{ league.currentPhase"
    - path: "app/components/PaintingProgress.vue"
      provides: "Painting leaderboard with currentPhase prop"
      contains: "currentPhase"
    - path: "app/components/views/DashboardView.vue"
      provides: "Dashboard with currentPhase prop binding"
      contains: ":currentPhase="
...
1. Line 10: Rename prop definition from `currentRound` to `currentPhase`:
   ```javascript
   currentPhase: {
     type: Number,
     default: 1
   ...
   ```
2. Search for any internal uses of `currentRound` in the component and rename to `currentPhase` (none found in template, but verify)

**DashboardView.vue:**
1. Line 226: Change prop binding from `:currentRound="league?.currentPhase || 1"` to `:currentPhase="league?.currentPhase || 1"`
...
Run: `grep -rn ":currentRound=" app/components/`
Expected: No matches (all prop bindings converted to :currentPhase=)

Run: `grep -n "currentRound:" app/components/*.vue`
Expected: No matches in prop definitions (all converted to currentPhase:)
  </verify>
  <done>
...
PaintingProgress accepts `currentPhase` prop instead of `currentRound`
...
grep -rn "currentRound:" app/components/ --include="*.vue"
...
3. **Prop definition check:**
```bash
grep -rn "currentRound:" app/components/ --include="*.vue"
```
Expected: No matches (all prop definitions use currentPhase)

4. **Prop binding check:**
```bash
grep -rn ":currentRound=" app/components/ --include="*.vue"
```
Expected: No matches (all prop bindings use :currentPhase=)
...
4. Default rules use "phase" terminology
5. Placeholder hints use "Phase 1"
6. PaintingProgress.vue uses `currentPhase` prop (not `currentRound`)
7. DashboardView.vue passes `:currentPhase` prop (not `:currentRound`)
8. Test suite shows no new failures
9. Build succeeds
10. grep verification confirms no remaining "round" in display contexts
</success_criteria>
