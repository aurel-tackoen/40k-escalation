...
// League phases table (progression periods in the escalation league)
...
export const players = pgTable('players', {
  ...
  joinedPhase: integer('joined_phase').default(1).notNull(), // Phase when player joined
  leftPhase: integer('left_phase'), // Phase when player left (null if active)
  createdAt: timestamp().defaultNow().notNull()
});
