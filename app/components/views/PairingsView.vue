<script setup>
  ...
  // State - Selected stage (defaults to current stage)
  const selectedStage = ref(null)

  // Watch for league changes to set initial stage
  watch(() => currentLeague.value, (league) => {
    if (league?.currentStage && !selectedStage.value) {
      selectedStage.value = league.currentStage
    }
  }, { immediate: true })
  ...
  const stageOptions = computed(() => {
    if (!currentLeague.value?.stages || !Array.isArray(currentLeague.value.rounds)) {
      return []
    }
    return currentLeague.value.stages.map(r => ({
      value: r.number,
      label: `Stage ${r.number} - ${r.name}`
    }))
  })

  const selectedStagePairings = computed(() => {
    if (!selectedStage.value || !props.pairings) return []
    return props.pairings.filter(p => p.stage === selectedStage.value)
  })

  const selectedStageUnpairedCount = computed(() => {
    if (!selectedStage.value || !props.players || !props.pairings) return 0
    const pairedPlayerIds = new Set()
    selectedStagePairings.value.forEach(pairing => {
  ...
  const unpairedPlayers = computed(() => {
    ...
    return getUnpairedPlayers(selectedStage.value)
  })

  const suggestedMethod = computed(() => {
    if (!selectedStage.value) return 'swiss'
    return suggestPairingMethod(selectedStage.value)
  })

  // Methods
  const changeRound = (roundNumber) => {
    selectedStage.value = roundNumber
  }

  // Methods
  const generatePairings = async (method) => {
    if (!currentLeague.value || !selectedStage.value) return
    ...
    try {
      let newPairings
      if (method === 'swiss') {
        newPairings = generateSwissPairings(selectedStage.value, currentLeague.value.id)
      } else if (method === 'random') {
        newPairings = generateRandomPairings(selectedStage.value, currentLeague.value.id)
      }

      // Send to API via store
      await leaguesStore.generatePairings(selectedStage.value, newPairings)
  ...
  const addManualPairing = async () => {
    if (!currentLeague.value || !manualForm.value.player1Id || !selectedStage.value) return
  ...
  const clearAllPairingsConfirmed = async () => {
    if (!selectedStage.value || selectedStagePairings.value.length === 0) return

    try {
      // Delete all pairings for the selected round
      const deletePromises = selectedStagePairings.value.map(pairing =>
        leaguesStore.deletePairing(pairing.id)
      )

      await Promise.all(deletePromises)

      toastSuccess(`Cleared all ${deletePromises.length} pairings for Round ${selectedStage.value}`)
...
<template>
  ...
  <div v-else-if="stageOptions.length === 0" class="text-center py-12">
  ...
  <div v-else class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <select
          ...
          @change="changeRound(selectedStage)"
        ...
        <button
          v-if="canManageLeague && selectedStagePairings.length > 0"
      ...
      <div class="text-sm text-gray-400">
        {{ selectedStagePairings.length }} pairings •
        {{ selectedStageUnpairedCount }} unpaired players
      </div>
    </div>

    <!-- Generate Pairings (Organizer Only) -->
    <div v-if="canManageLeague && selectedStageUnpairedCount > 0" class="card">
    ...
    <div class="space-y-3">
      ...
      <div v-if="selectedStagePairings.length === 0" class="card text-center py-8 text-gray-400">
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody>
            <tr
              v-for="pairing in selectedStagePairings"
              :key="pairing.id"
    ...
    <ConfirmationModal
      ...
      :title="`Clear All Pairings for Round ${selectedStage}?`"
      :message="`Are you sure you want to delete all ${selectedStagePairings.length} pairings for this round? This action cannot be undone.`"
