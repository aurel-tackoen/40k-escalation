...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!body.player1Id || !body.player2Id || body.stage === undefined) {
      throw createError({
        statusCode: 400,
        statusMessage: 'Player IDs and stage are required'
      })
    }
    ...
    if (body.leagueId && body.stage) {
      // Find the pairing for these two players in this stage
