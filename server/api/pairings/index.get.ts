...
/**
 * GET /api/pairings?leagueId=X&stage=Y
 * Fetch pairings for a league (optionally filtered by stage)
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const stage = query.stage ? parseInt(query.stage as string) : null
    ...
    if (stage !== null) {
      conditions.push(eq(pairings.stage, stage))
    }
