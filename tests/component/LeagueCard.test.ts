...
describe('LeagueCard', () => {
  ...
  describe('League Stats', () => {
    ...
    it('displays current phase for my-league variant', () => {
      const wrapper = mountWithMocks({
        league: mockMyLeague
      })
      expect(wrapper.text()).toContain('Phase 2 of 3')
      expect(wrapper.find('[data-testid="calendar-icon"]').exists()).toBe(true)
    })

    it('displays current phase for public variant without total', () => {
      const wrapper = mountWithMocks({
        league: { ...mockPublicLeague, phases: undefined },
        variant: 'public'
      })
